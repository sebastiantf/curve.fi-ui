(window["webpackJsonp"] = window["webpackJsonp"] || []).push([
    ["chunk-2d21b29d"], {
        bf36: function(e, r, n) {
            "use strict";
            n.r(r);
            n("f6b3");
            var t = n("080d");
            n("901e"), n("fd33"), n("337f");

            function o(e, r) {
                var n = Object.keys(e);
                if (Object.getOwnPropertySymbols) {
                    var t = Object.getOwnPropertySymbols(e);
                    r && (t = t.filter((function(r) {
                        return Object.getOwnPropertyDescriptor(e, r).enumerable
                    }))), n.push.apply(n, t)
                }
                return n
            }

            function c(e) {
                for (var r = 1; r < arguments.length; r++) {
                    var n = null != arguments[r] ? arguments[r] : {};
                    r % 2 ? o(Object(n), !0).forEach((function(r) {
                        a(e, r, n[r])
                    })) : Object.getOwnPropertyDescriptors ? Object.defineProperties(e, Object.getOwnPropertyDescriptors(n)) : o(Object(n)).forEach((function(r) {
                        Object.defineProperty(e, r, Object.getOwnPropertyDescriptor(n, r))
                    }))
                }
                return e
            }

            function a(e, r, n) {
                return r in e ? Object.defineProperty(e, r, {
                    value: n,
                    enumerable: !0,
                    configurable: !0,
                    writable: !0
                }) : e[r] = n, e
            }

            function A(e, r, n, t, o, c, a) {
                try {
                    var A = e[c](a),
                        i = A.value
                } catch (u) {
                    return void n(u)
                }
                A.done ? r(i) : Promise.resolve(i).then(t, o)
            }

            function i(e) {
                return function() {
                    var r = this,
                        n = arguments;
                    return new Promise((function(t, o) {
                        var c = e.apply(r, n);

                        function a(e) {
                            A(c, t, o, a, i, "next", e)
                        }

                        function i(e) {
                            A(c, t, o, a, i, "throw", e)
                        }
                        a(void 0)
                    }))
                }
            }

            function u(e, r) {
                if (null == e) return {};
                var n, t, o = l(e, r);
                if (Object.getOwnPropertySymbols) {
                    var c = Object.getOwnPropertySymbols(e);
                    for (t = 0; t < c.length; t++) n = c[t], r.indexOf(n) >= 0 || Object.prototype.propertyIsEnumerable.call(e, n) && (o[n] = e[n])
                }
                return o
            }

            function l(e, r) {
                if (null == e) return {};
                var n, t, o = {},
                    c = Object.keys(e);
                for (t = 0; t < c.length; t++) n = c[t], r.indexOf(n) >= 0 || (o[n] = e[n]);
                return o
            }
            var s = "data:image/png;base64,";

            function p(e) {
                var r = e.networkId,
                    o = e.preferred,
                    a = e.label,
                    A = e.iconSrc,
                    l = e.svg,
                    p = u(e, ["networkId", "preferred", "label", "iconSrc", "svg"]);
                return {
                    name: a || "Authereum",
                    svg: l,
                    iconSrc: A || s,
                    wallet: function() {
                        var e = i(regeneratorRuntime.mark((function e() {
                            var o, a, A, u;
                            return regeneratorRuntime.wrap((function(e) {
                                while (1) switch (e.prev = e.next) {
                                    case 0:
                                        return e.next = 2, Promise.all([n.e("chunk-065e770e"), n.e("chunk-76be3c62"), n.e("chunk-3c4e95ca")]).then(n.t.bind(null, "fe3c", 7));
                                    case 2:
                                        return o = e.sent, a = o["default"], A = new a(c({
                                            networkName: Object(t["f"])(r),
                                            blockedPopupRedirect: !1
                                        }, p)), u = A.getProvider(), e.abrupt("return", {
                                            provider: u,
                                            instance: A,
                                            interface: {
                                                name: "Authereum", connect: function() {
                                                    return u.enable()
                                                }, disconnect: function() {
                                                    A.destroy()
                                                }, address: {
                                                    get: function() {
                                                        return A.getAccountAddress()
                                                    }
                                                }, network: {
                                                    get: function() {
                                                        return Promise.resolve(r)
                                                    }
                                                }, balance: {
                                                    get: function() {
                                                        var e = i(regeneratorRuntime.mark((function e() {
                                                            var r;
                                                            return regeneratorRuntime.wrap((function(e) {
                                                                while (1) switch (e.prev = e.next) {
                                                                    case 0:
                                                                        return e.next = 2, A.isAuthenticated();
                                                                    case 2:
                                                                        return r = e.sent, e.abrupt("return", r && A.getBalance());
                                                                    case 4:
                                                                    case "end":
                                                                        return e.stop()
                                                                }
                                                            }), e)
                                                        })));

                                                        function r() {
                                                            return e.apply(this, arguments)
                                                        }
                                                        return r
                                                    }()
                                                }, dashboard: function() {
                                                    return Object(t["g"])("https://".concat(1 !== r ? "".concat(Object(t["f"])(r), ".") : "", "authereum.com/"))
                                                }
                                            }
                                        });
                                    case 7:
                                    case "end":
                                        return e.stop()
                                }
                            }), e)
                        })));

                        function o() {
                            return e.apply(this, arguments)
                        }
                        return o
                    }(),
                    type: "sdk",
                    desktop: !0,
                    mobile: !0,
                    preferred: o
                }
            }
            r["default"] = p
        }
    }
]);
//# sourceMappingURL=chunk-2d21b29d.c5ae311e.js.map